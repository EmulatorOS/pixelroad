function UnityProgress(dom) {
  ...
  this.SetMessage = function (message) {
    if (message == "Loading WebGL Player, Please wait...") {
      ...
      (m = message.match(/^Downloading data... \(([0-9]+)\/([0-9]+)\)/)) !==
      null
    ) {
  ...
  this.Update = function () {
    ...
    this.progressBar.width =
      progressFrameImg.width * Math.min(this.progress, 1);
...
function checkPannerSetVelocityError() {
  ...
  window.WEBAudio.audioInstances = new Proxy(
    window.WEBAudio.audioInstances,
    arrayChangeHandler
  );
}
